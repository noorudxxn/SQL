...
CREATE TABLE borrowers (borrower_id INT PRIMARY KEY,name VARCHAR(100),book_id INT,FOREIGN KEY (book_id) REFERENCES books(book_id));
...
SELECT borrowers.borrower_id,borrowers.name,books.book_id,books.title FROM borrowersLEFT JOIN books ON borrowers.book_id = books.book_id;
